...
class ExtentCOCODataset(Dataset):
    ...
    def _get_anno_file_name(self):
        ...
        if 'test' in self.dataset:
            ...
            return os.path.join(
                self.root,
                'annotations',
                'coco_wholebody_{}_v1.0.json'.format(
                    dataset.replace("2017", "")
                )
            )
...
class CocoRescoreDataset(ExtentCOCODataset):
    def __init__(self, cfg, dataset):
        ExtentCOCODataset.__init__(self, cfg, dataset)
